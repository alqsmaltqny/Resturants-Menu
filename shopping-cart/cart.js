...
return `
    ...
    <h4>${item.name}</h4>
    <!-- عرض سعر العنصر -->
    <p class="price">سعر: ${item.price}lt</p>
    <!-- عرض السعر الإجمالي للعنصر -->
    <p class="price">سعر الإجمالي: ${item.price * item.quantity}lt</p>
    </div></div>
    ...
    <button onclick="changeQuantity(${index}, -1)">-</button>
    <!-- زر لحذف العنصر -->
    <button onclick="removeItem(${index})" class="removebtn">حذف</button>
    </div>
    </div>
    </div>
  </div>
...
function checkout() {
    const message = cart.map(item => `${item.name}:  ${item.quantity}  ${item.price}lt x`).join('\n') + 
